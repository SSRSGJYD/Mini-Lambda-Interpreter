...
lookupExpr context varname = mytrace2 ("*** lookup expr: " ++ varname ++ " == " ++ (show e)) e 
      where e = case Map.lookup varname (exprMap context) of
                  Just ec -> Just $ head ec
                  _ -> Nothing

insertExpr :: String -> (Expr, ContextV) -> ContextV -> ContextV
insertExpr varname ec context@(ContextV adtMap constructorMap typeMap exprMap argList log) =  
  if containExpr context varname
  then mytrace2 ("*** insert expr: " ++ varname ++ " := " ++ (show $ fst ec)) ContextV adtMap constructorMap typeMap (Map.update (\x -> Just (ec : x)) varname exprMap) argList log
  else mytrace2 ("*** insert expr: " ++ varname ++ " := " ++ (show $ fst ec)) ContextV adtMap constructorMap typeMap (Map.insert varname [ec] exprMap) argList log

deleteExpr :: String -> ContextV -> ContextV
deleteExpr varname context@(ContextV adtMap constructorMap typeMap exprMap argList log) = 
  case Map.lookup varname exprMap of
    Just (t1:t2:t3) -> mytrace2 ("*** delete expr: " ++ varname) ContextV adtMap constructorMap typeMap (Map.update (\x -> Just (tail x)) varname exprMap) argList log
    Just [t] -> mytrace2 ("*** delete expr: " ++ varname) ContextV adtMap constructorMap typeMap (Map.delete varname exprMap) argList log
    _ -> mytrace2 ("*** delete Nothing") context

-- argument stack for lambda expression
pushArg :: (Expr, ContextV) -> ContextV -> ContextV
pushArg ec (ContextV adtMap constructorMap typeMap exprMap argList log) = mytrace2 ("### pushing arg:" ++ (show $ fst ec)) ContextV adtMap constructorMap typeMap exprMap (ec:argList) log

emptyArg :: ContextV -> Bool
emptyArg (ContextV adtMap constructorMap typeMap exprMap argList log) = null argList

firstArg :: ContextV -> (Expr, ContextV)
firstArg (ContextV adtMap constructorMap typeMap exprMap argList log) = mytrace2 ("### first arg:" ++ show (fst $ head argList)) head argList

popArg :: ContextV -> ContextV
popArg (ContextV adtMap constructorMap typeMap exprMap argList log) = mytrace2 ("### poping arg:" ++ show (fst $ head argList)) ContextV adtMap constructorMap typeMap exprMap (tail argList) log
...
prependLog newLog (ContextV adtMap constructorMap typeMap exprMap argList log) = ContextV adtMap constructorMap typeMap exprMap argList (newLog:log)

printLogs :: ContextV -> IO ()
printLogs (ContextV adtMap constructorMap typeMap exprMap argList log) = printStrLns log
